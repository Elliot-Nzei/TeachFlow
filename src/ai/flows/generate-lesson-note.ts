...
// This is the final output structure of the EXPORTED function
...
/**
 * The main exported function that orchestrates the lesson note generation.
 * @param input The generation parameters.
 * @returns The complete lesson note with metadata.
 */
export async function generateLessonNote(input: GenerateLessonNoteInput): Promise<GenerateLessonNoteOutput> {
  ...
  try {
    // The result from the flow will be just the note string.
    const result = await generateLessonNoteFlow(input);
    
    // The wrapper function adds the metadata.
    return {
      note: result.note,
      metadata: {
...
// This schema defines what the AI model ITSELF is expected to return.
// It should be simple and focused: just the note.
const AIPromptOutputSchema = z.object({
  note: z.string().describe("The generated lesson note in Markdown format for the single week requested."),
});
...
const prompt = ai.definePrompt({
  ...
  // The output schema for the prompt MUST match what we ask for in the instructions.
...
// The Genkit flow itself. It can have complex internal logic, but its
// final output should be predictable.
const generateLessonNoteFlow = ai.defineFlow(
  {
    name: 'generateLessonNoteFlow',
    inputSchema: GenerateLessonNoteInputSchema,
    // The output of the flow itself is simple.
    outputSchema: AIPromptOutputSchema,
  },
  async (input) => {
    // Call the AI prompt. The result will conform to AIPromptOutputSchema.
    const { output } = await prompt(input);
    
    if (!output || !output.note) {
      throw new Error('No valid note content was generated by the AI model.');
    }
    
    // Return the simple object. The calling function will handle adding metadata.
